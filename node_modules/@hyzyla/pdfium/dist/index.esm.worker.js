...
class WorkerChannel {
    ...
    call(req, transfer = []) {
        ...
        const message = Object.assign({ id }, req);
...
class PDFiumWorkerPage {
    constructor(channel, handle, pageIndex) {
        this.channel = channel;
        this.handle = handle;
        this.number = pageIndex;
    }
    getText() {
        return this.channel.call({ type: "page.getText", page: this.handle });
    }
    getOriginalSize() {
        return this.channel.call({ type: "page.getOriginalSize", page: this.handle });
    }
    render(options = {}) {
...
class PDFiumWorkerDocument {
    constructor(channel, handle) {
        this.channel = channel;
        this.handle = handle;
    }
    getPageCount() {
        return this.channel.call({ type: "doc.getPageCount", doc: this.handle });
    }
    getPage(pageIndex) {
        return __awaiter(this, void 0, void 0, function* () {
            const result = yield this.channel.call({
                type: "doc.getPage",
                doc: this.handle,
                pageIndex,
            });
            return new PDFiumWorkerPage(this.channel, result.page, result.pageIndex);
        });
    }
    pages() {
        return __asyncGenerator(this, arguments, function* pages_1() {
            const count = yield __await(this.getPageCount());
            for (let i = 0; i < count; i++) {
                yield yield __await(yield __await(this.getPage(i)));
            }
        });
    }
    destroy() {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.channel.call({ type: "doc.destroy", doc: this.handle });
        });
    }
}
class PDFiumWorkerClientBase {
    constructor(channel) {
        this.channel = channel;
    }
    loadDocument(bytes, password) {
        return __awaiter(this, void 0, void 0, function* () {
            // Transfer the bytes' underlying buffer to avoid a copy when it's exclusive
            // to this call. Callers passing a shared/view buffer should clone first.
            const transferable = bytes.buffer instanceof ArrayBuffer ? [bytes.buffer] : [];
            const result = yield this.channel.call({ type: "loadDocument", bytes, password }, transferable);
            return new PDFiumWorkerDocument(this.channel, result.doc);
        });
    }
    destroy() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                yield this.channel.call({ type: "destroy" });
            }
            finally {
...
function spawnWorkerChannel(opts) {
    return __awaiter(this, void 0, void 0, function* () {
        const blob = new Blob([opts.workerSource], { type: "text/javascript" });
        const objectUrl = URL.createObjectURL(blob);
        const worker = new Worker(objectUrl, { type: "module" });
        const channel = new WorkerChannel(worker, objectUrl);
        // Transfer wasmBinary's buffer to avoid copying it across the boundary.
        const transfer = opts.init.wasmBinary ? [opts.init.wasmBinary] : [];
        yield channel.call(Object.assign({ type: "init" }, opts.init), transfer);
        return channel;
    });
}

class PDFiumWorkerClient extends PDFiumWorkerClientBase {
    static spawn(options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!options || (!options.wasmUrl && !options.wasmBinary)) {
                throw new Error("PDFiumWorkerClient.spawn: wasmUrl or wasmBinary is required");
            }
            const channel = yield spawnWorkerChannel({
                ...
                init: { wasmUrl: options.wasmUrl, wasmBinary: options.wasmBinary },
            });
            return new PDFiumWorkerClient(channel);
        });
    }
}

export { PDFiumWorkerClient, PDFiumWorkerDocument, PDFiumWorkerPage };
