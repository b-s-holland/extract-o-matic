  ...
  for (const page of pages) {
    for (const block of page.blocks || []) {
      ...
      for (const paragraph of block.paragraphs || []) {
        const rawLines = extractParagraphLines(paragraph, pageWidth, pageHeight);
        ...
        fragments.push({
          ...
          blockId,
          text,
          bbox: paragraph.boundingBox ? normalizeBbox(paragraph.boundingBox.vertices, pageWidth, pageHeight) : unionBboxes(lines.map((line) => line.bbox)),
      ...
      if (paragraphRefs.length) {
        blocks.push({
          ...
          order: blockOrder,
          paragraphRefs,
          bbox: block.boundingBox ? normalizeBbox(block.boundingBox.vertices, pageWidth, pageHeight) : null,
